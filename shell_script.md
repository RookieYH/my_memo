...
- 字符串处理
  - `${变量:-默认值}`，变量不存在或为空则返回默认值，`${变量:=默认值}`，变量不存在或为空则变量将被设为默认值并返回默认值，`${变量:+默认值}`，变量不存在或为空，返回空，否则返回默认值
  - `${变量:位置起点:要截取的长度}`，字符串截取，位置从0开始，`${变量名:位置起点}`，从起点开始取到末尾，起点可以从右边算起，`0-n`表示从右起第 n 位，也可以前面加空格` -n`，第三个参数如果是负数如` -n`，则表示截取到倒数第 n+1 位，省略要截取的长度，则截取到末尾
  - `${#变量名称}`计算变量值字符串的长度
  - `${var/1234/1111}`， 如果变量中有符合样式的字符串，则使用替换字符串替代，只替换第一个符合样式的字符串
  - `${变量//Pattern/替换字符串}`，如果变量中有符合样式的字符串，则使用替换字符串替代，替换全部符合样式的字符串
  - `${变量/#Pattern/替换字符串}`，如果变量开始部分符合样式的字符串，则使用替换字符串替代匹配，`${变量/%Pattern/替换字符串}`，如果变量，末尾部分符合样式的字符串，则使用替换字符串替代匹配
...
- 数组
  ...
  - `${array_name[@]/Pattern/string}`，`${array_name[@]#Pattern}`，`${array_name[@]%Pattern}`， `${parameter^Pattern}`，等字符串处理也适用于数组，和参数数组 @ ，对每个元素分别执行操作，返回结果数组
...
- `[ expression ] `或`test expression `，测试语句，与 if、case、while 搭配，返回“真(0)”或“假(非0)”，在任何一个运算符、圆括号或者方括号等操作符额前后 至少需要留有一个空格，否则可能会出错
  ...
  - `[[ expression ]]`也可以用来执行测试语句，可以直接用 **()** 改变执行顺序而不需转义，用 **&&** 表示逻辑与 **||** 表示逻辑或，**!** 表示取反
    - **[[...]]** 结构中，`=~`表示正则匹配，`pattern='[[:space:]]*(a)?b'; [[ $line =~ $pattern ]]`，右边的为正则表达式，右边能匹配左边的一部分就返回真，前式中，只要 line 中含有 b 就为真，右边如果被 **""** 或 **''** 包裹，则作为普通字符而非正则
  ```bash
  [ ! "$s1" -le 0 ] #s1大于0
